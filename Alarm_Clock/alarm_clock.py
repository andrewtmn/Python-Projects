...
class AlarmClock():
    ...
    def __init__(self, master):
        ...
        self._date_time = tk.Label(master, text=str(self._current_time), \
            font=12)
        ...
        self._alarm_time = tk.Label(master, font=10, \
             text="Set your alarm in minutes", relief=tk.RAISED)
        ...
        minutes_label = tk.Label(self._set_alarm_frame, \
             text="Set Minutes for Alarm:")\
             .pack(side=tk.LEFT, expand=True, fill = tk.X, padx=5)

        self._alarm_entry = tk.Entry(self._set_alarm_frame, \
             textvariable=self._time_set)
        self._alarm_entry.pack(side=tk.LEFT, expand=True, fill = tk.X)

        self._reset_alarm = tk.Button(self._set_alarm_frame, \
            text="Reset Alarm", command=self.reset)
        self._reset_alarm.pack(side=tk.RIGHT, padx=5)

        self._set_alarm = tk.Button(self._set_alarm_frame, \
            text="Set Alarm", command=self.set_time)
        self._set_alarm.pack(side=tk.RIGHT, padx=5)

        self.redraw()

    def redraw(self):
        """ Refreshes the current time clock so that time is updated 
        every second """
        self._current_time = time.asctime(time.localtime(time.time()))
    ...
    def set_time(self):
        ...
        try:
            ...
            self._alarm_time.config(\
                text="That's not a valid value for minutes.")
            self._time_set.set(0)
        return
    

    def check_if_time(self):
        """ checks if alarm time has been reached; alerts user if
         that is the case"""
